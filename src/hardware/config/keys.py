...
- Defines which keys exist (KeyId enum).
- Maps each key to its LED matrix zone (KEY_ZONES).
- Optionally assigns debug colors for each key (KEY_COLORS).
"""
...
class KeyId(IntEnum):
    ...
    You can rename them to real note names if desired
...
# x = 0 and x = 31 are reserved as borders.
# 5 keys use x = 1..30, each key occupies 6 columns.
...
# Optional debug colors for each key (RGB)
